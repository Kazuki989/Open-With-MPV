...
chrome.webRequest.onHeadersReceived.addListener(d => {
  ...
  if (!type) {
    ...
    // subtitles detection
    type = SUBTITLE_TYPES.find(s => (new RegExp('.' + s + '\\b')).test(href));
    ...
    chrome.scripting.executeScript({
      ...
      if (r) {
        ...
        if (links.length === 1) {
          ...
          await filtersubtitlelinks(links)
...
async function filtersubtitlelinks(links) {
  SUBTITLES_LINKS = []
  await links.forEach(async link => {
    type = SUBTITLE_TYPES.find(s => (new RegExp('.' + s + '\\b')).test(link));  
      if (type && await checklanguage(link) == 'eng') {
        SUBTITLES_LINKS.push(link);
      }
  })
...
async function checklanguage(link) {
  ...
  lang = franc(cleanedData.toString().trim())
...
function asTuples(value) {
  ...
  for (trigram in dictionary) {
    if (own$1.call(dictionary, trigram)) {
